...
class Tipocategoria extends AUTH_Controller
{
    ...
    public function index()
    {
        ...
        $data['dataPosisi'] = $this->M_tipocategoria->select_all();

        $data['page'] = "tipocategoria";
        $data['judul'] = "Tipo Categoría";
        $data['deskripsi'] = "Administrar Tipo Categoría";
    ...
    {
        $this->form_validation->set_rules('des_tip_categoria', 'des_tip_categoria', 'trim|required');

        $data = $this->input->post();
        if ($this->form_validation->run() == TRUE) {
            $result = $this->M_tipocategoria->insert($data);
    ...
    public function update()
    {
        ...
        $id = trim($_POST['id']);
        $data['dataPosisi'] = $this->M_tipocategoria->select_by_id($id);
    ...
    public function prosesUpdate()
    {
        ...
        $data = $this->input->post();
        if ($this->form_validation->run() == TRUE) {
            $result = $this->M_tipocategoria->update($data);

            if ($result > 0) {
                $out['status'] = '';
                $out['msg'] = show_succ_msg('Dato actualizado exitosamente', '18px');
            } else {
                $out['status'] = '';
                $out['msg'] = show_succ_msg('Error al actualizar el dato', '18px');
            }
        } else {
            $out['status'] = 'form';
            $out['msg'] = show_err_msg(validation_errors());
    ...
    {
        $id = $_POST['id'];
        $result = $this->M_tipocategoria->delete($id);

        if ($result > 0) {
            echo show_succ_msg('Dato eliminado exitosamente', '18px');
        } else {
            echo show_err_msg('Error al eliminar el dato', '18px');
        }
    }
